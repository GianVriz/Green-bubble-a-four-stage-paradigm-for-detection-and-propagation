...
The BSADF (Back Sup-ADF) is compared with the Kolmogorov-Smirnov Change Point Detection Model (KS-CPM). The disruptive path is initially examined, followed by 1000 simulations.
  ...
  tryCatch({
    ...
    test <-
      processStream(ss,
                    "Kolmogorov-Smirnov",
                    ARL0 = 500,
                    startup = 20) #startuo 20%, ARL0=500 are default values
...
# Empirical analysis, detection phase
...
test <-
  processStream(
    ...
    ARL0 = 500,
    startup = 20
  ) #20 initial sample and ARL0=500 are default values.
...
# Empirical analysis, Propagation phase
...
#ADF for text data
summary(ur.df(
  Climate[, "Carbon_p"], #lag=12 for seasonality
  type = "drift",
  lags = 12,
  selectlags = "AIC"
))
...
#Dataset lagge values of text variables only (scaled values)
...
```{r, warning=FALSE}
#Dummy codification for explosive behaviours
Data_2$dummy_Renixx <-
  as.factor(
    ifelse(
