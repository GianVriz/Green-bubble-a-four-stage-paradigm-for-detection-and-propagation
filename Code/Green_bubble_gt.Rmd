  ...
  tryCatch({
    ...
    test <-
      processStream(ss,
                    "Kolmogorov-Smirnov",
                    ARL0 = 500,
                    startup = 17) #startuo 17%, ARL0=500
...
test <-
  processStream(
    ts(df$rate, freq = 52, start = decimal_date(ymd("2005-1-1"))),
    "Kolmogorov-Smirnov",
    ARL0 = 5000,
    startup = round(dim(df)[1] * 17 / 100)
  ) #17% initial sample, ARL0 5000 as proposed
...
test <-
  processStream(
    ...
    "Kolmogorov-Smirnov",
    ARL0 = 500,
    startup = dim(renixx_m)[1] * 17 / 100
  ) #1 initial sample, ARL0=500
...
#Oil WTI Futures
Oil <-
  read.csv("Economic variables/Crude Oil WTI Futures Historical Data_2005.csv")
...
#Geopolitical index
data_gpr_export <-
  read_excel("Economic variables/data_gpr_daily_recent.xls")
geo_i <- data_gpr_export %>% dplyr::select(6, 3)
colnames(geo_i)[1] <- "Date"
geo_i2 <-
  geo_i[geo_i$Date <= "2022-12-30" & geo_i$Date >= "2005-01-01",]
...
#Correlation-plot
corrplot(
  cor(Climate[c(1:216), c(2:8)], method = 'kendall'),
...
corrplot(
  cor(Finance[c(1:216), (2:length(Finance[1, ]))], method = 'kendall'),
  hclust.method = 'ward.D2',
  addrect = 3,
  rect.col = 'gold',
  rect.lwd = 4,
...
#Correlation plot
corrplot(
  cor(Data[1:95, c(2:length(Data[1, ]))], method = 'kendall'),
  diag = FALSE,
  method = 'square',
  bg = 'black',
  col = c('lightblue', 'green'),
...
corrplot(
  cor(Data[96:178, c(2:length(Data[1, ]))], method = 'kendall'),
  diag = FALSE,
  method = 'square',
  bg = 'black',
  col = c('lightblue', 'green'),
...
corrplot(
  cor(Data[179:215, c(2:length(Data[1, ]))], method = 'kendall'),
  diag = FALSE,
  method = 'square',
  bg = 'black',
  col = c('lightblue', 'green'),
...
corrplot(
  cor(Data[, c(2:length(Data[1, ]))], method = 'kendall'),
  diag = FALSE,
  method = 'square',
  bg = 'black',
  col = c('lightblue', 'green'),
...
p2 <-
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Natural Disasters') + labs(fill =
                                                                                          'Stage') + ylab('Density')
p3 <-
  ggplot(
    Data_2,
  ...
  ) + geom_density(alpha = 0.4) + xlab('Carbon Price') +
  guides(color = "none") + geom_density(alpha = 0.4) + labs(fill = 'Stage') + ylab('Density')
p4 <-
  ggplot(
    Data_2,
    aes(
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Energy Index') + labs(fill =
                                                                                     'Stage') + ylab('Density')
p5 <-
  ggplot(
    Data_2,
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Global Warming') + labs(fill =
                                                                                       'Stage') + ylab('Density')
p6 <-
  ggplot(Data_2,
         aes(
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('New Technology') + labs(fill =
                                                                                       'Stage') + ylab('Density')
p7 <-
  ggplot(
    Data_2,
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Energy Shares') + labs(fill =
                                                                                      'Stage') + ylab('Density')
p8 <-
  ggplot(Data_2,
         aes(
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Carbon Tax') + labs(fill =
                                                                                   'Stage') + ylab('Density')
grid.arrange(p1,
             p2,
             p3,
...
#Scaling
Data_2$Oillag <-
  scale(c(0, diff(Data_2$Oillag)), center = TRUE, scale = TRUE)
Data_2$EPUlag <-
  scale(c(0, diff(Data_2$EPUlag)), center = TRUE, scale = TRUE)
Data_2$MSCIlag <-
  scale(c(0, diff(Data_2$MSCIlag)), center = TRUE, scale = TRUE)
Data_2$GEOlag <-
  scale(c(0, diff(Data_2$GEOlag)), center = TRUE, scale = TRUE)
```
...
p9 <-
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('MSCI') + labs(fill =
                                                                             'Green bubble')
p10 <-
  ggplot(Data_2,
         aes(
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('EPU') + labs(fill =
                                                                            'Green bubble')
p11 <-
  ggplot(Data_2,
         aes(
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Oil price') + labs(fill =
                                                                                  'Green bubble')
...
p12 <-
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('Geo Index') + labs(fill =
                                                                                  'Green bubble')
...
p13 <-
  ...
  guides(color = "none") + geom_density(alpha = 0.4) + xlab('RENIXX Index') + labs(fill =
                                                                                     'Green bubble')
...
grid.arrange(
  ...
  p13,
  ncol = 2,
  top = textGrob("Renixx Global Index Explosive Evets", gp = gpar(fontsize =
                                                                    20, font = 3))
)
```
...
set.seed(123)
seeds <- vector(mode = "list", length = 42)
for (i in 1:41)
  seeds[[i]] <- sample.int(1000, 15)
seeds[[42]] <- sample.int(1000, 1)
tuneLength.num <- 15
...
```{r}
#Logit model with only the RENIXX indes.
model_3 <-
...
#Oil price
Oil_pred <-
  read.csv("Predictions/Oil_pred.csv",
           header = TRUE,
           comment.char = "#")
...
#MSCI
MSCI_pred <-
  read.csv("Predictions/MSCI World_pred.csv",
           header = TRUE,
           comment.char = "#")
...
MSCI_predm <- MSCI_predm[127:138, ]
MSCI_predm <- rbind(MSCI_m[, 1:2], MSCI_predm)
#GEPUCURRENT
GEPUCURRENT_pred <-
  read.csv("Predictions/GEPUCURRENT_pred.csv",
           header = TRUE,
           comment.char = "#")
...
#Geopolitical index
data_gpr_pred <- read_excel("Predictions/GPI_pred.xls")
...
geo_predm <- rbind(geo_m[, 1:2], geo_predm)
#Renixx index
Renixx_pred <-
  read.csv("Predictions/Renixx_pred.csv",
           header = TRUE,
           comment.char = "#")
...
Renixx_predm <- rbind(renixx_m[, 1:2], Renixx_predm)
#Dataset
data_pred <-
  data.frame(
...
for (i in 96:(sample - 1)) {
  #lag
  (ss <- list())
  ss <- AddAutoAr(ss, data_pred[1:i, c('Renixx')])
...
#Istantaneous causality
vm = VAR(data_pred[1:96, c(6:8, 1)],
         p = 1,
         type = 'trend',
         season = 12)
...
```{r, warning=FALSE}
#Arimax with lag values
data_pred2 <-
  cbind(data_pred$Renixx, lag(data_pred[, c(6)]))[2:length(data_pred[, 1]), ] #lag values to prevent overconfidence
...
```{r, warning=FALSE}
#SMAPE
Metrics::smape(predictions_uni[, 1], data_pred$Renixx[97:108]) * 100
...
#ARIMAX lag-values
dm.test(m[, 1], m[, 3], alternative =  "greater", h = 1)
dm.test(m[, 2], m[, 3], alternative =  "greater", h = 1)
dm.test(m[, 4], m[, 3], alternative =  "greater", h = 1)
dm.test(m[, 5], m[, 3], alternative =  "greater", h = 1)
#ARIMAX actual data
dm.test(m[, 1], m[, 4], alternative =  "greater", h = 1)
dm.test(m[, 2], m[, 4], alternative =  "greater", h = 1)
dm.test(m[, 3], m[, 4], alternative =  "greater", h = 1)
dm.test(m[, 5], m[, 4], alternative =  "greater", h = 1)
#Theil's U
TheilU(data_pred$Renixx[97:108], predictions_uni[, 1])
TheilU(data_pred$Renixx[97:108], predictions_uni[, 2])
TheilU(data_pred$Renixx[97:108], predictions_uni[, 3])
TheilU(data_pred$Renixx[97:108], predictions_uni[, 4])
TheilU(data_pred$Renixx[97:108], predictions_uni[, 5])
```
...
ggplot(data_pred[65:sample, ],
       aes(x = as.Date(Date),
           y = Renixx),) +
  geom_line(aes(
    y = c(data_pred$Renixx[65:96], predictions_uni[, 3]),
    color = 'Prediction'
  )) +
  geom_ribbon(
    aes(
      ymin = c(data_pred$Renixx[65:96], lower_uni[, 3]),
      ymax = c(data_pred$Renixx[65:96], up_uni[, 3])
    ),
    alpha = 0.1,
    fill = "green",
    color = "green",
...
rownames(data_pred) <- NULL
data_pred$Dummy1 <-
  c(rep(1, 36), rep(0, 59), rep(0, 83), rep(1, 38), rep(0, 12))
data_pred$Dummy2 <-
  c(rep(0, 36), rep(1, 59), rep(0, 83), rep(0, 38), rep(1, 12))
```
...
colnames(predictions_multi) <- c('ARIMA', 'VAR', 'BSTS')
up_multi <- as.data.frame(matrix(nrow = iteration, ncol = models))
colnames(up_multi) <- c('ARIMA', 'VAR', 'BSTS')
lower_multi <- as.data.frame(matrix(nrow = iteration, ncol = models))
colnames(lower_multi) <- c('ARIMA', 'VAR', 'BSTS')
```

```{r}
#Granger causality
vm = VAR(data_pred[1:216, c(1:13)],
         p = 1,
         type = 'both',
         season = 12)
model_summary(vm)
...
#Dummy variables
exo <-
  as.matrix(data_pred[1:216, c('Dummy1', 'Dummy2')], ncol = 2, nrow = 216)
colnames(exo) <- c('exo1', 'exo2')
#ARIMA+dummy
arimad <-
  auto.arima(all_series[1:216, c('Renixx')], seasonal = TRUE, xreg = exo)
#New dummy variables
...
#VECM
all_series <-
  ts(data_pred[1:216, c('Renixx', 'Energy_i', 'Carbon_t', 'Oil_p')], start =
       c(2005, 1), frequency = 12)
#Model selection
var <-
  VARselect(all_series,
            type = "both",
            ...
            season = 12)
p <- var$selection["FPE(n)"]
exo <- as.matrix(data_pred[1:216, c('Dummy1', 'Dummy2')], ncol = 2, nrow =
                   11)
colnames(exo) <- c('exo1', 'exo2')
#Cointegration
nlags = max(3, p) - 1
...
#BSTS
(ss <- list())
ss <- AddAutoAr(ss, data_pred[1:216, c('Renixx')])
ss <- AddLocalLevel(ss, data_pred[1:216, c('Renixx')])
ss <- AddSeasonal(ss, data_pred[1:216, c('Renixx')], nseasons = 12)
bsts.model2 <- bsts(
  Renixx ~ .,
  state.specification = ss,
  niter = 1000,
  data = data_pred[1:216, c('Renixx', 'Dummy1', 'Dummy2')],
  ping = 0,
  seed = 2016,
  expected.model.size = 3
)
...
##Saving
predictions_multi[, 3] <- pred_bayesian2$mean
lower_multi[, 3] <- pred_bayesian2$interval[1, ]
up_multi[, 3] <- pred_bayesian2$interval[2, ]
```
...
```{r, warning=FALSE}
#SMAPE
Metrics::smape(predictions_multi[, 1], data_pred$Renixx[217:228]) * 100
Metrics::smape(predictions_multi[, 2], data_pred$Renixx[217:228]) * 100
Metrics::smape(predictions_multi[, 3], data_pred$Renixx[217:228]) * 100
#RMSE
Metrics::rmse(predictions_multi[, 1], data_pred$Renixx[217:228])
Metrics::rmse(predictions_multi[, 2], data_pred$Renixx[217:228])
Metrics::rmse(predictions_multi[, 3], data_pred$Renixx[217:228])
```

```{r, warning=FALSE}
#DM
m <- predictions_multi - data_pred$Renixx[217:228]
dm.test(
  m[, 1],
  m[, 2],
  alternative =  "greater",
...
#Theil's U
TheilU(data_pred$Renixx[217:228], predictions_multi[, 1])
TheilU(data_pred$Renixx[217:228], predictions_multi[, 2])
TheilU(data_pred$Renixx[217:228], predictions_multi[, 3])
```
...
ggplot(data_pred[185:sample,],
       aes(x = Date,
           y = Renixx), ) +
  geom_line(aes(
    y = c(data_pred$Renixx[185:216], predictions_multi[, 2]),
    color = 'Prediction'
  )) +
  geom_ribbon(
    aes(
      ymin = c(data_pred$Renixx[185:216], lower_multi[, 2]),
      ymax = c(data_pred$Renixx[185:216], up_multi[, 2])
    ),
    alpha = 0.1,
    fill = "green",
    color = "green",
...
all_series <-
  ts(
    Data_3[c(1:216), c('Renixx', 'OFR', 'Oil_p')],
    start = c(2005, 1),
    end = c(2022, 12),
    frequency = 12
  )
...
#xtable(var_OFR$varresult$OFR,digits=c(3,3,3,3,3))
#xtable(var_OFR$varresult$Renixx,digits=c(3,3,3,3,3))
#xtable(var_OFR$varresult$Oil_p,digits=c(3,3,3,3,3))
```
